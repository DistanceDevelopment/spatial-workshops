---
title: Estimating variance (df covariate models)
...
The models we investigate here have estimated abundance as the response and have covariates at the observer level in the detection function, so we calculate the variance using standard GAM methodology and combine uncertainty with detection function uncertainty by the delta method.
...
varest_map <- dsm.var.gam(dsm.nb.xy, split(predgrid, 1:nrow(predgrid)), off.set=predgrid$off.set)
pp <- plot(varest_map, observations=FALSE, plot=FALSE)
pp <- pp + scale_fill_viridis()
pp <- pp + geom_point(aes(x=x,y=y, size=abundance.est), data=dsm.nb$data[dsm.nb$data$abundance.est>0,])
print(pp)
```
