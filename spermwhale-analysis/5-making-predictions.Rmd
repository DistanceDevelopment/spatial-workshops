...
First need to load the variables into a "stack" from their files on disk. At this point we arbitrarily choose the time periods of the SST and NPP rasters.
...
What were the per model abundance estimates:
```{r per-model-est}
allmodels_Nhat
```

Note here that the quasi-Poisson models are a whole order of magnitude different from the other models.
...
This plot sucks because the two quasi-Poisson models throw out the scale, let's throw them out...
...
Note we'll write the results out to a raster once we've calculated the uncertainty attached to these abundance estimates.

# Save the predictiond data
