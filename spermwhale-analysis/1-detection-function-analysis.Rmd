...
```{r ht}
Nhat_hr <- dht(models$hr$ddf, region.table, sample.table, obs.table)
```

Now replacing `p` with the values from the `df_hr_ss` model we can make an estimate for that model too:
```{r ht-cov}
Nhat_hr_ss <- dht(models$hr.ss$ddf, region.table, sample.table, obs.table)
```
...
df_hr <- models$hr
save(df_hr_ss, df_hr, distdata, region.table, sample.table, obs.table, file="df-models.RData")
```
