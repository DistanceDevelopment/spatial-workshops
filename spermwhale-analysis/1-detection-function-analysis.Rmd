...
total_length <- segs@data$Length
ht_est <- (5.285e+11/(2*sum(total_length)*df_hr$ddf$meta.data$width))* sum(df_hr$ddf$data$size/predict(df_hr$ddf)$fitted)
ht_est
```

```{r ht-cov}
ht_est_cov <- (5.285e+11/(2*sum(total_length)*df_hr$ddf$meta.data$width))* sum(df_hr$ddf$data$size/predict(df_hr_ss$ddf)$fitted)
ht_est_cov
```