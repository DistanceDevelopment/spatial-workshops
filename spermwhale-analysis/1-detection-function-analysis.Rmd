...
The observations are located in a "geodatabase" we created in Arc. We want to pull out the "Sightings" table and from that we just want the `@data` element. We can check it has the correct format using `head`:
...
df_hr <- models$hr
save(df_hr_ss, df_hr, file="df-models.RData")
```
...
First we need to load the effort data from the geodatabase, this is in the "Segments" table. We can then sum the total line length (recalling that $a=2wL$ where $w$ is the truncation and $L$ is the total line length):
```{r get-effort}
segs <- readOGR("Analysis.gdb", "Segments")
total_length <- sum(segs@data$Length)
a <- 2 * 6000 * total_length
```

The total area of the EEZ is $A=5.285e+11$, we can extract the probabilities of detection using `predict(model)$fitted` and the group/cluster sizes are stored in the model object in `model$ddf$data$size`.

We now have everything we need to make an abundance estimate:
```{r ht}
A <- 5.285e+11
p <- predict(df_hr$ddf)$fitted
sizes <- df_hr$ddf$data$size
ht_est <- (A/a) * sum( sizes/p )
ht_est
```

Now replacing `p` with the values from the `df_hr_ss` model we can make an estimate for that model too:
```{r ht-cov}
p <- predict(df_hr_ss$ddf)$fitted
ht_est_cov <- (A/a) * sum( sizes/p )
ht_est_cov
```
...
```{r ht-perception}
p <- predict(df_hr$ddf)$fitted * 0.46
ht_est_p <- (A/a) * sum( sizes/p )
ht_est_p
```

```{r ht-cov-perception}
p <- predict(df_hr_ss$ddf)$fitted * 0.46
ht_est_cov_p <- (A/a) * sum( sizes/p )
ht_est_cov_p
```
