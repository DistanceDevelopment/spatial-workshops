...
pp_raster <- raster(nrow=nrow(predictorStack), ncol=ncol(predictorStack),
...
save_raster <- stack(c(predictorStack, pp_raster))
# name the new, last, layer in the stack
names(save_raster)[length(names(save_raster))] <- "Nhat_nb_xy"
```
We can then save that object to disk as a raster file:
```{r write-raster}
writeRaster(save_raster, "abundance_raster.img", datatype="FLT4S")
```
...
We'll need to use the prediction grid again when we calculate uncertainty in the next practical, so let's save that object now to save time later.

```{r save-rdata}
save(predgrid, file="predgrid.RData")
```
