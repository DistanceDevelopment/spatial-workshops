...
<div class="boxout">
As promised, here is an example using the data from GIS and the `dht()` function. Note that this code may not run until you've got the data as in practical 2.
...
Now we can call the `dht` function that will estimate the abundance for us, given the tables we've constructed:
```{r ht}
...
Now that was a lot of output! But looking at the `Summary for individuals` section and the `Abundance` table we can see the differences between the two models' abundance estimates.
