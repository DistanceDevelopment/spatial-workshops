...
<p>If you wanted to fit a detection function using Distance 7, you would have to ask my wife. </p>

<img src=DSC00648.JPG width=640 height=480>
...
The `summarize_ds_models()` function expects a character vector of object names as its input. We can do that with the two models that I fitted like so:
```{r model-list}
models <- c("df_hn", "df_hr_ss_size")
```
...
```{r df-results}
model_table <- summarize_ds_models(models)
kable(model_table, digits=3, caption = "Comparison of half normal and hazard rate with sea state and group size.")
```
