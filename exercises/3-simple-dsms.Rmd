---
title: 'Practical 3: Simple density surface models'
...
- Fitting a density surface model using `dsm()`
  - Knowing what the objects that go into a `dsm()` call
  - Understand the role of the response in the `formula=` argument
...
The example code below uses the `df_hr` detection function in the density surface models. You can substitute this for your own best model as you go, or copy and paste the code at the end and see what results you get using your model for the detection function.
...
dsm_nb_xy <- dsm(count~s(x,y),
                 ddf.obj=df_hr, segment.data = segs, observation.data=obs,
...
Try copying the code that fits the model `dsm_nb_xy` and make a model `dsm_nb_xy_ae` that replaces `count` for `abundance.est` in the model formula. Compare the results of summaries, plots and checks between this and the count model.
...
- We used the `nb()` and `tw()` response distributions above for negative binomial and Tweedie distributions respectively. These distributions can also be specified using `negbin()` and `Tweedie()`. Use the manual pages in R to find out what the difference between the alternative specifications are.
