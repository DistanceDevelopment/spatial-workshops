...
```{r nb-xy}
dsm_nb_xy <- dsm(count~s(x,y, bs="ts") +
                       s(Depth, bs="ts") +
                       s(DistToCAS, bs="ts") +
                       s(SST, bs="ts") +
                       s(EKE, bs="ts") +
...
```{r plot-nb-xy}
plot(dsm_nb_xy, pages=1, scale=0)
```
...
```{r nb-x-y}
dsm_nb_x_y <- dsm(count~s(x, bs="ts") +
                        s(y, bs="ts") +
                        s(Depth, bs="ts") +
                        s(DistToCAS, bs="ts") +
                        s(SST, bs="ts") +
...
```{r compare-depth}
par(mfrow=c(1,2))
plot(dsm_nb_xy, select=2)
plot(dsm_nb_x_y, select=3)
```
...
model_list <- list(dsm_nb_x_y, dsm_nb_xy)
library(plyr)
summary_table <- ldply(model_list, summarize_dsm)
row.names(summary_table) <- c("dsm_nb_x_y", "dsm_nb_xy")
```
